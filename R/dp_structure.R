...
#' class and attributes. For examples, use `dpcode_add` to generate a dp_make.R file
#' @param data_files_read object generated from `dpinput_read()`
#' containing links to input data
#' @param config data product config file from `dpconf_get`
...
dp_structure <- function(data_files_read, config, output = list(), metadata = list()) {
  ...
  data_object$README <- readme_get(data_object, general_note = config$readme_general_note)
  ...
  attr(data_object, "dp_name") <- dpname_get(data_object = data_object)
