...
For a new project, start by initializing the project using `dp_init`,
...
Note that the second function call relies on “AWS_ACCESS_KEY_ID” and
“AWS_SECRET_ACCESS_KEY” to be available in the environment when the
function is being hydrated. Do not pass keys or secrets directly to
`creds_set*`. Instead, use environment variables as above or a password
manager package such as [keyring](https://github.com/r-lib/keyring).
...
dpcode_add(project_path = dp_repo)
```
...
synced_map <- dpinput_sync(conf = config, input_map = input_map, verbose = TRUE)
...
dp_commit(project_path = ".", commit_description = "First dp build: only input data")
