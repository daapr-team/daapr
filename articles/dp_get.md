...
daapr provides a minimalist and uniform interface for retrieving data
products and the associated metadata no matter where the data product is
housed. Currently, s3, local, and LabKey are three types of remote
repositories for housing data products.

This vignette covers reading a data product that already exists. To
build and deploy one, see the [new project
...
leverages the concept of `boards` for organizing data into **data
...
If the data product is stored under a prefix within the bucket rather
than at the bucket root, pass that prefix as well. The prefix must end
...
For LabKey, you need to provide the server url and the path of the study
folder on that server. A `cache_alias` can optionally be given to name
...
your script or inside any files that are version-controlled. It is
highly recommended to establish a habit of using environment variables
in conjunction with secret management packages like `keyring`.
Credentials can be set as an environment variable via an `.Renviron`
file or via a secret manager package such as `keyring`. For AWS
credentials, users can also store these through standard aws
configuration files and load them into their session using the
...
For LabKey, a single API key is needed. See the [LabKey documentation on
API
...
Local boards need no credentials, so `creds` can be omitted when
connecting to one.
