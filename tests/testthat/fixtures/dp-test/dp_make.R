...
list(
  ...
  # Derive datatopic1
  tar_target(subjects, derive_subjects(data_files_read = data_files_read, config = config)),
  tar_target(bor, derive_bor(data_files_read = data_files_read, config = config)),

  # Structure data object
  tar_target(
    name = data_object,
    command =  dp_structure(data_files_read, config,
      output = list(subjects = subjects, bor = bor
      ),
      metadata = list())
  ),
