...
**Goal:** Get the right subset of all data synced to remote. To this end, using `dpinput_map` we check the content of the data uploaded into `input_file`, if needed, change names, remove dataset that need to be excluded from the sync.
...
# input_map <- dpinput_syncflag_reset(input_map = input_map, input_id = c("dataset_id_to_be_reset1","dataset_id_to_rest2")
...
```{r dpinput_write}
dpinput_write(project_path = ".", input_d = input_d)
```


# Editing `dp_make.R` and building the data product
...
targets::tar_visnetwork(script = "./dp_make.R")
```
